...
require_once JPATH_ROOT . '/templates/affinity/libreries/extHelper.inc.php';
				...
				switch($view) {
					...
					*/
					...
					case 'categories':
						...
						// $output .= '<div class="list-group-item phocadownloadcategories'.(int)$iSize.'">
						// 							<a href="'. JRoute::_($link).'" '.$targetOutput.'>'. $textOutput.' <i class="fa fa-chevron-circle-right" aria-hidden="true"></i></a>
						// 						</div>';
					...
					case 'filelist':
						...
						$query = 'SELECT a.id, a.title, a.alias, a.filename_play, a.filename_preview, a.link_external, a.image_filename, a.filename, a.publish_up, c.id as catid, a.confirm_license, c.title as cattitle, c.alias as catalias,'
						. ' CASE WHEN CHAR_LENGTH(a.alias) THEN CONCAT_WS(\':\', a.id, a.alias) ELSE a.id END as slug,'
						...
						if (!empty($items)) {
							...
							// $output .= '<li class="list-group-item header-item"><strong>Lista download file</strong></li>';
